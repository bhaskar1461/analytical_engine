...
import { generateSip } from '@/lib/api-client';
...
export default function SipPage() {
  ...
  const submit = async () => {
    ...
    try {
      const response = await generateSip({
      ...
      setError(cause instanceof Error ? cause.message : 'Unable to generate SIP plan');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <main className="space-y-6">
      <section className="rounded-2xl border border-white/10 bg-white/5 p-4 sm:p-6">
        <h1 className="text-2xl font-bold text-white sm:text-3xl">Smart SIP Planner</h1>
        <p className="mt-2 text-sm text-slate-300">
          Volatility-aware monthly allocation suggestions with rebalance triggers.
        </p>
        <p className="mt-2 text-xs text-slate-400">
          Educational and non-binding output. It should not be treated as financial advice.
        </p>
        ...
        >
          {loading ? 'Generating...' : 'Generate SIP'}
        </button>
        ...
        {result ? (
          <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/40 p-3 sm:p-4">
            <p className="text-sm text-slate-200">Budget: {formatInr(result.monthlyBudgetInr)}</p>
            <p className="text-sm text-slate-200">
              Expected drawdown: {result.expectedDrawdown.toFixed(1)}%
            </p>
            <p className="mt-1 text-xs text-slate-400">
              Illustration only. Use your own due diligence before investing.
            </p>
              ...
              {result.allocations.map((item: any) => (
                ...
                >
                  ...
                  <p className="break-words text-slate-300">
                    {item.sector} · Trust {item.trustScore.toFixed(1)}
                  </p>
                </div>
              ))}
            </div>

            <div className="mt-4 space-y-1 text-xs text-cyan-200">
              {result.rebalanceTriggers.map((trigger: string) => (
